<div>
                                ...
                                <tr>
                                    ...
                                    <td class="flex items-center px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                                        ...
                                        <div class="flex-shrink-0 h-10 w-15 ">
                                            @if ($categoryy->image)
                                                <img class="object-cover w-20 h-10 rounded-sm" src="{{url($categoryy->image) }}" alt="">
                                                    {{-- src="{{ Storage::url($brand->image) }}" storage//storage/categories/default.jpg  en la bd esta puesto esto 	/storage/categories/default.jpg > --}}
                                                    {{-- para que muestre correcto Storage::url($brand->image) en la BD deberia estar puesto categories/default.jpg--}}

                                                    {{-- url($brand->image) muestra tal como es la ruta en la bd,  esta puesto esto 	/storage/categories/default.jpg --}}
                                                    {{--  {{ Storage::disk("s3")->url($brand->image) }} --}}
                                            @else
                                                <img class="object-cover w-20 h-10 rounded-sm"
                                                    src="/storage/categories/default2.jpg" alt="">
                                            @endif
                                        </div>
                                        <div class="ml-4">
                                            {{ $categoryy->name }}
                                    ...
                                    <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                        ...
                                        <a wire:click="edit({{ $categoryy }})" class="btn btn-green"><i class="fa-solid fa-pen-to-square"></i></a>
                                        <a class="btn btn-red" wire:click="$emit('deleteCategoryy', {{ $categoryy->id }})" >
    ...
    <x-jet-dialog-modal wire:model="open_edit">
        ...
        <x-slot name="content">
            ...
            <div class="mb-4">
                Estado
                <x-jet-input type="checkbox" wire:model="category.state" />
        ...
        Livewire.on('deleteCategory', categoryId => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    Livewire.emitTo('admin.category-list','delete',categoryId);
